...
MOBILE
INOVAÇÃO & GESTÃO
PYTHON E ORIENTAÇÃO A OBJETOS > COLLECTIONS
COLLECTIONS
Objetivos:
...
Crie um arquivo chamado contas.py na pasta src do projeto banco. Crie uma classe chamada Contas que herde da classe abstrata MutableSequence:
...
if __name__=='__main__':
    contas = Contas()
Nossa sequência só deve permitir adicionar elementos que sejam do tipo Conta. Vamos acrescentar essa validação nos métodos __setitem__ e insert. Caso o valor não seja uma Conta, vamos lançar um TypeError com as devidas mensagens de erro:
...
if __name__=='__main__':
    ...
    arquivo.close()
Vamos criar uma laço for para ler cada linha do arquivo e construir um objeto do tipo ContaCorrente.
...
arquivo.close()    
Nossa classe Contas implementa MutableSequence. Isso quer dizer que ela é iterável já que MutableSequence implementa o protocolo __iter__ através do método __getitem__. Vamos iterar através de uma laço for nosso objeto contas e pedir para imprimir o saldo e o valor do imposto de cada uma delas:
...
(Desafio Opcional) Refaça o exercício utilizando MutableMapping ao invés de MutableSequence.
